 ...
 */
	public function testBuildStatementWithLockingHint() {
		...
		$sql = $db->buildStatement($query, $this->model);
		$expected = 'SELECT  TOP 1 id FROM "user" AS [User]  WITH (UPDLOCK)   WHERE 1 = 1   ORDER BY [id] ASC';
		$this->assertEquals($expected, $sql);

		$sql = $db->buildStatement(array('offset' => 15) + $query, $this->model);
		$expected = 'SELECT id FROM "user" AS [User]  WITH (UPDLOCK)   WHERE 1 = 1   ORDER BY [id] ASC  OFFSET 15 ROWS FETCH FIRST 1 ROWS ONLY';
		$this->assertEquals($expected, $sql);
	}
